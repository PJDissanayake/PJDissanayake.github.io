  ...
  useEffect(() => {
    ...
    const drawParticles = () => {
      ...
      particles.forEach((particle, i) => {
        ...
        const glowIntensity = distance < mouseRadius ? (1 - distance / mouseRadius) * 0.5 : 0
        ctx.beginPath()
        ctx.arc(particle.x, particle.y, particle.size + glowIntensity * 2, 0, Math.PI * 2)
        ctx.fillStyle = `rgba(56, 189, 248, ${particle.opacity + glowIntensity})`
        ctx.fill()
        ...
        particles.slice(i + 1).forEach((other) => {
          ...
          if (pDistance < 120) {
            ...
            ctx.strokeStyle = `rgba(56, 189, 248, ${0.2 * (1 - pDistance / 120)})`
            ctx.lineWidth = 0.5
            ctx.stroke()
          }
        })
        ...
        if (distance < mouseRadius) {
          ...
          ctx.strokeStyle = `rgba(56, 189, 248, ${0.3 * (1 - distance / mouseRadius)})`
          ctx.lineWidth = 0.8
          ctx.stroke()
        }
      })
